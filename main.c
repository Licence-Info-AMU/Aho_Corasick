...
*/
char ** trismots(char ** argv,int start,int end){
	char **mots=copy_argv(argv,start,end);
	sort_words(mots,end-start);
	return mots;
}
...
int main(int argc, char* argv[]){
	...
	char** mots=trismots(argv,2,argc);
	show_argv(mots,argc-2);
	printf("\n");

	int nb_etats; 
	char ** prefix=genere_prefix(mots,0,argc-2,&nb_etats);
